...
packConstructor =
  [ p (S.Constr "Cons") (constructorTriple "Cons" 0 2)
...
desugarSCs pscs = desugarSC <$> pscs
  where
  desugarSC (S.Func fname arg body) = AST.Func fname arg (desugarAST body)

desugarAST :: S.CorePAST -> AST.CoreExpr
desugarAST (S.Var n) = AST.Var n
desugarAST (S.App f e) = AST.App (desugarAST f) (desugarAST e)
desugarAST cons@(S.Constr _) =
  mkConstructor $ unsafePartial $ fromJust
